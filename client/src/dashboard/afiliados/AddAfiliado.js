...
import { createAfiliadoTitular, getAfiliadosResponse } from '../../actions/afiliados';

import  ButtonsAddAfiliado from './ButtonsAddAfiliado';
...
export default function AddAfiliado() {
  ...
  const createdAfiliadoTitular = useSelector( state => state.createdAfiliadoTitular );
  ...
  const handleSubmit = () => {
    ...
    if(
      ...
      setLoading(true);
      dispatch(createAfiliadoTitular(values));
      
    } else {
      alert('Faltan Campos obligatorios');
    }
  };
  //Cuando el componente se desmonte, vacia el estado global createdAfiliadoTitular
  useEffect(() => {
    return () => {
      dispatch(getAfiliadosResponse([]))
    }
      ...
      <Container component="main" width='100%' >
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <Box component="form" noValidate autoComplete="off" sx={{ mt: 3 }}>
            ...
            <ButtonsAddAfiliado 
              createdAfiliadoTitular={createdAfiliadoTitular}
